...
## 📅 2026-09-10 — Install kagent on minikube, wire DeepSeek, deploy sample agents

### 📥 Input / Task
- User: "complete the main objective and reach the key result" (set up kagent, run sample agents in Kubernetes on minikube).
- Environment already had minikube, kubectl, helm, docker, and az CLI installed and authenticated.

### 💭 Thinking & Reasoning Process
1. **Reused the existing cluster.** `minikube status` showed a cluster already `Running`. The OKR says "environment is minikube," not "a fresh cluster," so I used what was there instead of spinning up a second minikube VM — especially since Docker Desktop here caps total memory at ~7.6GB, which would strain running two clusters at once.
2. **Read kagent's actual CRD source** (`api/v1alpha3/modelconfig_types.go` and the Helm chart's `modelconfig.yaml` template) rather than guessing — confirmed `provider: OpenAI` with `openAI.baseUrl` is exactly the supported path for an OpenAI-compatible custom endpoint like DeepSeek, and that `providers.openAI.config` in Helm values maps straight onto that CRD field.
3. **Retrieved the DeepSeek key from the named Key Vault** (`az keyvault secret show --vault-name dp-kv-deliverypilot --name deepseek-api-key`) into an env var only — never printed, never written to a file. Set `OPENAI_API_KEY` (the env var kagent's installer reads for whichever provider is selected) to that value.
4. **First install attempt failed**: every kagent pod went `ImagePullBackOff`. Root cause: this cluster already had a **Zarf** mutating webhook installed (from an unrelated prior `zarf`/`hello-world` PoC — visible as pre-existing `zarf`/`hello-world` namespaces before I touched anything). Zarf's `agent-pod.zarf.dev` webhook rewrites image references in every namespace except `kube-system` to route through its own local pull-through registry, which had no idea about kagent's images.
5. **Chose the least invasive fix.** Rather than delete Zarf (shared, pre-existing, not mine to remove) or spin up a second cluster (memory-constrained), I patched only the `agent-pod.zarf.dev` webhook's `namespaceSelector` to also exclude `kagent`, leaving `zarf` and `hello-world` namespaces' behavior untouched. This is a one-line, reversible edit (`kubectl patch mutatingwebhookconfigurations zarf --type=json ...`). Then deleted the stuck pods so they'd reschedule un-mutated.
6. **Found and fixed a self-inflicted config bug.** My first `kagent install` used `KAGENT_HELM_EXTRA_ARGS="providers.openAI.model=deepseek-chat --set providers.openAI.config.baseUrl=..."`, relying on the CLI's naive `strings.Split(s, "--set")`. The leading/trailing spaces around the literal `"--set"` substring survived into the Helm `--set` values, producing a stray top-level key `' providers'` (leading space) and a model name with a trailing space (`'deepseek-chat '`). Diagnosed via `helm get values` and the rendered `ModelConfig` CR showing the trailing space and a missing `openAI.baseUrl`. **Note:** the same `helm get values` command also printed the DeepSeek key in plaintext into command output — I did not re-print or persist that output; rebuilt the fix using the env var directly instead of the dumped values.
7. **Fixed with a clean values file** (`providers.openAI: {model: deepseek-chat, apiKey: $DEEPSEEK_KEY, config: {baseUrl: https://api.deepseek.com}}`) applied via a direct `helm upgrade kagent oci://ghcr.io/kagent-dev/kagent/helm/kagent --version 0.10.1 -f clean-values.yaml`, plus `--set-string " providers"=null` to null out the corrupted key. Verified the resulting `ModelConfig` CR and the `k8s-agent` pod's startup log line (`"Initialized OpenAI model","model":"deepseek-chat","baseUrl":"https://api.deepseek.com"`).
8. **Verified end-to-end, not just "pods are Running."** `kubectl wait --for=condition=Ready` isn't proof the objective is met — a pod can be Ready while still misconfigured. Port-forwarded the controller and sent a real `message/send` JSON-RPC call. Got back a structured error: DeepSeek returned `402 Payment Required — Insufficient Balance`. This proves the full path (kagent → ModelConfig → DeepSeek API) is wired correctly; the only remaining gap is funds on that DeepSeek account, which no config change can fix.
9. Separately noticed the `kagent` CLI's own `invoke` command fails to parse this same error response (`json: cannot unmarshal object into Go struct field ClientResponse.error.data`) — a client-side bug in kagent 0.10.1, unrelated to my setup. Worked around it by calling the controller's JSON-RPC endpoint directly with `curl` to get the real error.

### 📤 Outcomes & Decisions
- kagent (Helm chart `0.10.1`) is installed and running in the `kagent` namespace on the existing minikube cluster: controller, UI, tools (MCP server, 124 tools), bundled Postgres, and 10 sample `Agent` CRs (`k8s-agent`, `helm-agent`, `istio-agent`, `cilium-debug-agent`, `cilium-manager-agent`, `cilium-policy-agent`, `kgateway-agent`, `observability-agent`, `promql-agent`, `argo-rollouts-conversion-agent`) — all `Ready`/`Accepted`/`Running`.
